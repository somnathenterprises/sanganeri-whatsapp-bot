...
router.get('/inventory', async (req, res) => {
    try {
      ...
      const inventory = products.map(product => {
              ...
              const totalBooked = variants.reduce((s, v) => s + v.booked, 0);
              const totalAvailable = variants.reduce((s, v) => s + v.available, 0);
              return { id: product.id, title: product.title, image, variants, totalStock, totalBooked, totalAvailable };
      });

      res.json({ products: inventory, total: inventory.length });
    } catch (e) {
          res.status(500).json({ error: e.message });
    }
});
