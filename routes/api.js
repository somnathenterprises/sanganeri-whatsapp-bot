...
router.get('/inventory', async (req, res) => {
  try {
    ...
    const productsRes = await axios.get(baseURL + '/products.json', { headers, params: { limit, status: 'active', fields: 'id,title,image,images,variants' } });
    const products = productsRes.data.products || [];
    const ordersRes = await axios.get(baseURL + '/orders.json', { headers, params: { limit: 250, fulfillment_status: 'unfulfilled', status: 'open', fields: 'line_items' } });
    ...
    for (const product of products) {
      ...
      for (const variant of (product.variants || [])) {
        ...
        inventory.push({ product_id: product.id, variant_id: variant.id, product_title: product.title, variant_title: variant.title, sku, image, available_qty: available, booked_qty: booked, net_qty: available - booked, price: variant.price });
      }
    }
