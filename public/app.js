...
window.handleTopbarRefresh = function() {
...
window.switchPage = function(page) {
  ...
  document.querySelectorAll('.mobile-tabbar button').forEach(function(b) { b.classList.remove('active'); });
  const section = document.getElementById('page-' + page);
  if (section) section.style.display = (page==='chats') ? '' : 'block';
  const navItem = document.querySelector('.nav-item[data-page="' + page + '"]');
  if (navItem) navItem.classList.add('active');
  const mBtn = document.querySelector('.mobile-tabbar button[data-page="' + page + '"]');
  if (mBtn) mBtn.classList.add('active');
  const titles = { orders:'Orders', inventory:'Inventory', chats:'WhatsApp Chats', cod:'COD Verifications', templates:'Send Templates', settings:'Settings' };
  const titleEl = document.getElementById('topbar-title');
  if (titleEl) titleEl.textContent = titles[page] || page;
  const searchEl = document.getElementById('order-search');
  if (searchEl) searchEl.style.display = (page==='orders') ? '' : 'none';
...
function renderStats(orders) {
  ...
  setEl('stat-revenue', '₹' + revenue.toFixed(0));
  setEl('stat-cod', cod);
...
window.filterOrders = function() {
  ...
  const status = (document.getElementById('filter-financial')||{value:''}).value;
  let filtered = allOrders;
  if (q) filtered = filtered.filter(function(o) {
    const n = o.customer ? ((o.customer.first_name||'')+(o.customer.last_name||'')).toLowerCase() : '';
    return n.includes(q) || (o.name||'').toLowerCase().includes(q);
  });
  if (status) filtered = filtered.filter(function(o) { return o.financial_status===status; });
...
window.openOrderDrawer = async function(orderId) {
  ...
  try {
    ...
    const lineItemsHtml = (o.line_items||[]).map(function(li) {
      return '<tr><td>'+esc(li.title)+(li.variant_title?' ('+esc(li.variant_title)+')':'')+'</td><td>x'+li.quantity+'</td><td style="text-align:right;">₹'+parseFloat(li.price).toFixed(2)+'</td></tr>';
    }).join('');
    if (body) body.innerHTML =
      ...
      '<div class="drawer-section">' +
        ...
        (o.customer&&o.customer.email?'<div class="detail-row"><span class="detail-label">Email</span><span style="font-size:12px;">'+esc(o.customer.email)+'</span></div>':'') +
      '</div>' +
      '<div class="drawer-section">' +
        ...
        '<div class="detail-row" style="margin-top:8px;border-top:1px solid #eee;padding-top:6px;"><b>Total</b><b style="color:#1a73e8;">₹'+parseFloat(o.total_price).toFixed(2)+'</b></div>' +
      '</div>' +
  ...
  try {
    ...
    el.innerHTML = convs.map(function(c) {
      ...
      return '<div class="chat-item" onclick="loadConversation(' + JSON.stringify(c.phone) + ')">' +
        '<div class="chat-avatar">'+esc((c.phone||'?').slice(-2))+'</div>' +
...
window.loadConversation = async function(phone) {
  ...
  try {
    ...
    if (replyEl) replyEl.placeholder = 'Reply to +' + phone + '...';
  } catch(e) {
  ...
  try {
    ...
    el.innerHTML = verifications.map(function(v) {
      ...
      return '<div class="cod-card">' +
        ...
        '<div style="font-weight:600;">₹'+esc(String(v.amount||''))+'</div>' +
        (v.status==='pending'?'<button onclick="resendVerification('+JSON.stringify(v.orderId)+')" style="margin-top:8px;padding:6px 12px;border:1px solid #ccc;border-radius:4px;cursor:pointer;">Resend</button>':'') +
        '</div>';
    }).join('');
  } catch(e) {
...
function loadSettings() {
  ...
  if (aiToggle) aiToggle.addEventListener('change', function() {
    api('/settings', { method: 'POST', body: JSON.stringify({ ai_enabled: aiToggle.checked }) }).catch(function(){});
  });
}
...
function initApp() {
  ...
  const searchEl = document.getElementById('order-search');
  if (searchEl) searchEl.addEventListener('input', window.filterOrders);
  const filterEl = document.getElementById('filter-financial');
  if (filterEl) filterEl.addEventListener('change', window.filterOrders);
